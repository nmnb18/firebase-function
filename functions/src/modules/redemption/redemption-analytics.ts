...
export const redemptionAnalytics = createCallableFunction<void, any>(
  async (_, auth) => {
    ...
    const metrics = {
      ...
      // Last 7 days
      last7days: {
        total: redemptions.filter(
          (r) => r.created_at && new Date(r.created_at) >= last7
        ).length,
      ...
      last30days: {
        total: redemptions.filter(
          (r) => r.created_at && new Date(r.created_at) >= last30
        ).length,
        redeemed: redemptions.filter(
          (r) =>
            r.status === "redeemed" &&
      ...
      repeat_customers: (() => {
        const customerCounts = redemptions.reduce(
          (acc, r) => {
      ...
      average_redemption_value: (() => {
        ...
        return redeemed.length > 0
          ? Math.round(
              redeemed.reduce((sum, r) => sum + (r.points || 0), 0) /
                redeemed.length
            )
          : 0;
      })(),
      ...
      average_processing_time_minutes: (() => {
        const redeemed = redemptions.filter(
          (r) => r.status === "redeemed" && r.created_at && r.redeemed_at
        );
    ...
    const customerRedemptions = redemptions.reduce(
      (acc, r) => {
        if (!acc[r.user_id]) {
          acc[r.user_id] = {
            user_id: r.user_id,
